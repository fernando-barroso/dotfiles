...
- OpenCode server runs as a launchd service on port 4096, Tailscale interface only (server only)
- Wrapper script (`~/.local/bin/opencode-server`) reads password from file and resolves Tailscale IP at launch
